...
In the figure above, all three possible sources of noise are indicated with the same parameter symbol $\sigma_2$. Not because, noise is thought to be identical, but because in ReMeta, the researcher must decide for one metacognitive noise source and hence only a single metacognitive noise parameter $\sigma_2$ will be estimated. By default — and hence our recommendation — ReMeta assumes a noisy-report architecture. 